        ...
        UIView.animateKeyframes(withDuration: self.transitionDuration(using: transitionContext),
                                ...
                                animations: {
                                    UIView.addKeyframe(withRelativeStartTime: 0,
                                                       relativeDuration: 1,
                                                       animations: {
                                                        let tranlation = CGAffineTransform(translationX: source.view.frame.height / 2, y: -source.view.frame.width)
                                                        let rotation = CGAffineTransform(rotationAngle: -CGFloat(Double.pi/2))
                                                        source.view.transform = rotation.concatenating(tranlation)
                                                       })
